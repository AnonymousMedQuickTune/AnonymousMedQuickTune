...
class DenseModel(nn.Module):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ("conv0", nn.Conv3d(in_channels, config["init_features"], kernel_size=5, # kernel size changed from 7 to 5.
                                        ...
                                        bias=False)),
                    ("norm0", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=config["init_features"])),
        ...
        in_channels = config["init_features"]
        block_config = (config["num_layers_block1"], config["num_layers_block2"], config["num_layers_block3"], config["num_layers_block4"])
