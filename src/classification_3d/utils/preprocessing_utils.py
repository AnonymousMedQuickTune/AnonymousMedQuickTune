...
def resize_depth_dimension(image, segmentation):
    """
    Resize only the depth (Z) dimension of images to reduce memory usage.
    
    This function reduces only the depth dimension to a maximum of 96 voxels,
    while preserving the width (X) and height (Y) dimensions unchanged.
    This preserves X-Y resolution (important for diagnostic features) while 
    reducing computational cost in the depth dimension.
    ...
    Note:
        - If the depth dimension is already <= 96 voxels, no resizing is performed
        - Uses linear interpolation for images and nearest neighbor for segmentations
    ...
    print(f"Resized depth dimension from (x, y, z) = {sitk_size_before} to (x, y, z) = {sitk_size_after}")
...
def resize_worcdatabase_images(image, segmentation, dataset_name):
    """
    Resize WORC database images to reduce memory usage.
    
    This function applies dataset-specific resizing strategies:
    
    For LIPO, DESMOID, and LIVER:
    - Height (Y) and Width (X) dimensions to 256 voxels
    - Depth (Z) dimension to 32 voxels
    
    For GIST, MELANOMA, and CRLM:
    - Height (Y) and Width (X) dimensions to 256 voxels
    - Depth (Z) dimension to 96 voxels
    
    This reduces computational cost while maintaining reasonable spatial resolution.
    All resizing is performed using zoom interpolation (linear for images, nearest neighbor for segmentations).
    
    Args:
        image (sitk.Image): The image to resize
        segmentation (sitk.Image): The segmentation to resize
        dataset_name (str): Name of the dataset ("lipo", "desmoid", "liver", "gist", "melanoma", or "crlm")
    ...
    # Print in consistent (x, y, z) format to match other print statements
    print(f"{dataset_name} dataset: Resized from (x, y, z) = {sitk_size_before} to (x, y, z) = {sitk_size_after}")
