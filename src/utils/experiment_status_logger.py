...
class ExperimentStatusLogger:
    """
    Logger for tracking experiment status using separate files for NePS and evaluation.
    """
    ...
    def __init__(self, experiment_base_dir: str):
        ...
        self.neps_status_file = self.status_dir / "neps_status.txt"
        ...
        self.neps_status = self._load_neps_status()
    ...
    def _load_neps_status(self) -> Dict:
        """Load NePS status from file or create new if not exists."""
        if self.neps_status_file.exists():
            try:
                with open(self.neps_status_file, 'r') as f:
                    content = f.read()
        ...
        # Create new NePS status structure
        return {
            'started': None,
            'finished': None,
            'last_updated': None,
    ...
    def update_neps_progress(self, outer_fold: int, inner_folds_completed: int, total_inner_folds: int):
        ...
        """
        if outer_fold not in self.neps_status['outer_folds_progress']:
            self.neps_status['outer_folds_progress'][outer_fold] = {
                'status': 'in_progress',
                'inner_folds_completed': 0,
        ...
        self.neps_status['outer_folds_progress'][outer_fold]['inner_folds_completed'] = inner_folds_completed
        self.neps_status['outer_folds_progress'][outer_fold]['total_inner_folds'] = total_inner_folds
        
        # Check if this outer fold is completed
        if inner_folds_completed == total_inner_folds:
            self.neps_status['outer_folds_progress'][outer_fold]['status'] = 'completed'
        
        # Update total completed outer folds
        completed_count = sum(1 for fold_info in self.neps_status['outer_folds_progress'].values() 
                            if fold_info['status'] == 'completed')
        self.neps_status['completed_outer_folds'] = completed_count
        
        self._save_neps_status()
    

    
    def set_total_outer_folds(self, total: int):
        """Set the total number of outer folds for the experiment."""
        if self.neps_status['started'] is None:
            self.neps_status['started'] = datetime.now().isoformat()
        
        self.neps_status['total_outer_folds'] = total
        self._save_neps_status()
    
    def mark_neps_finished(self):
        """Mark the NePS experiment as finished."""
        self.neps_status['finished'] = datetime.now().isoformat()
        self._save_neps_status()
    ...
    def _save_neps_status(self):
        ...
        self.neps_status['last_updated'] = datetime.now().isoformat()
    ...
    def _format_neps_status_text(self) -> str:
        ...
        lines.append(f"Started: {self.neps_status.get('started', 'Not started')}")
        lines.append(f"Finished: {self.neps_status.get('finished', 'Not finished')}")
        lines.append(f"Last Updated: {self.neps_status.get('last_updated', 'Never')}")
        lines.append("")
        
        total_outer = self.neps_status['total_outer_folds']
        completed_outer = self.neps_status['completed_outer_folds']
        ...
        for fold_num in range(1, total_outer + 1):
            if fold_num in self.neps_status['outer_folds_progress']:
                fold_info = self.neps_status['outer_folds_progress'][fold_num]
                if fold_info['status'] == 'completed':
...
class InnerFoldProgressLogger:
    ...
    def _extract_path_info(self, pipeline_directory):
        """
        Extract outer fold number, config number, and base directory from NePS pipeline directory.
        
        Args:
            pipeline_directory (str): Full pipeline directory path
            
        Returns:
            tuple: (outer_fold, config_num, base_dir) where outer_fold is 1-based and config_num is the NePS config number
        """
