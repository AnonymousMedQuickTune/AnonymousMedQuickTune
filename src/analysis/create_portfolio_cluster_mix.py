...
class PortfolioCreator:
    ...
    def _create_curves_dataframe(self, results: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        MIX APPROACH: Calculates averaged curve per config, then repeats it for each inner fold.
        This gives stable performance prediction while maintaining individual cost entries."""
        curves_data = []
        outer_fold_dirs = self._find_all_outer_folds()
        ...
        for idx, result in enumerate(results, start=1):  # Start enumeration at 1
            ...
            if all_fold_curves:
                ...
                # MIX APPROACH: Repeat the averaged curve for each inner fold
                # This gives stable performance prediction while maintaining individual cost entries
                num_inner_folds = result.get("_num_inner_folds", 1)
