...
def analyze_training_validation_metrics(neps_output_dir):
    ...
    with open(analysis_file, "w") as f:
        ...
        log_print(
            f"Training Accuracy: {df_mean['train_acc'].mean():.2f}% ± {df_mean['train_acc'].std():.2f}%",
            f,
        )
        log_print(
            f"Validation Accuracy: {df_mean['val_acc'].mean():.2f}% ± {df_mean['val_acc'].std():.2f}%",
            f,
        )
        log_print(
            f"Accuracy Gap (Train-Val): {df_mean['acc_gap'].mean():.2f}% ± {df_mean['acc_gap'].std():.2f}%",
            f,
        )

        log_print(
            f"\nTraining Loss: {df_mean['train_loss'].mean():.2f} ± {df_mean['train_loss'].std():.4f}",
            f,
        )
        log_print(
            f"Validation Loss: {df_mean['val_loss'].mean():.2f} ± {df_mean['val_loss'].std():.4f}",
            f,
        )
        log_print(
            f"Loss Gap (Val-Train): {df_mean['loss_gap'].mean():.2f} ± {df_mean['loss_gap'].std():.4f}",
            f,
        )

        log_print(
            f"\nTraining F1: {df_mean['train_f1'].mean():.2f}% ± {df_mean['train_f1'].std():.2f}%",
            f,
        )
        log_print(
            f"Validation F1: {df_mean['val_f1'].mean():.2f}% ± {df_mean['val_f1'].std():.2f}%",
            f,
        )
        log_print(
            f"F1 Gap (Train-Val): {df_mean['f1_gap'].mean():.2f}% ± {df_mean['f1_gap'].std():.2f}%",
            f,
        )

        log_print(
            f"\nTraining Precision: {df_mean['train_precision'].mean():.2f}% ± {df_mean['train_precision'].std():.2f}%",
            f,
        )
        log_print(
            f"Validation Precision: {df_mean['val_precision'].mean():.2f}% ± {df_mean['val_precision'].std():.2f}%",
            f,
        )
        log_print(
            f"Precision Gap (Train-Val): {df_mean['precision_gap'].mean():.2f}% ± {df_mean['precision_gap'].std():.2f}%",
            f,
        )

        log_print(
            f"\nTraining Recall: {df_mean['train_recall'].mean():.2f}% ± {df_mean['train_recall'].std():.2f}%",
            f,
        )
        log_print(
            f"Validation Recall: {df_mean['val_recall'].mean():.2f}% ± {df_mean['val_recall'].std():.2f}%",
            f,
        )
        log_print(
            f"Recall Gap (Train-Val): {df_mean['recall_gap'].mean():.2f}% ± {df_mean['recall_gap'].std():.2f}%",
            f,
        )
...
def analyze_validation_test_generalization(neps_output_dir, test_metrics):
    ...
    with open(analysis_file, "w") as f:
        ...
        metrics_file = (
            results_dir / f"config_{best_config_id}" / "logging" / "metrics.csv"
        )
        if not metrics_file.exists():
