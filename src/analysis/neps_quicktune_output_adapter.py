...
class NePSQuickTuneAdapter:
    ...
    def __init__(self, input_path: str, output_dir: str):
        ...
        self.neps_output = self.input_path / "NePS_output"  # Add NePS_output to path when needed
    ...
    def parse_neps_output(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                config_dict["final_accuracy"] = -row['objective_to_minimize']  # Assuming objective is negative accuracy
    ...
    def create_dataframes(
        ...
        for idx, result in enumerate(results, start=1):  # Start enumeration at 1
            ...
            try:
                ...
                k_folds = len([d for d in os.listdir(config_dir) 
                             if d.startswith("fold_") and os.path.isdir(os.path.join(config_dir, d))])
    ...
    def get_fold_metrics(self, config_idx: int, k_folds: int) -> List[np.ndarray]:
        ...
        for fold in range(k_folds):
            metrics_path = os.path.join(config_dir, f"fold_{fold}", "logging", "metrics.csv")
            try:
    ...
    def save_dataframes(
        self, config_df: pd.DataFrame, curves_df: pd.DataFrame, 
        cost_df: pd.DataFrame, meta_df: pd.DataFrame
    ) -> None:
        """
...
def merge_neps_runs(
    ...
    logging.info(f"Successfully merged {len(processed_runs)} NePS runs into {portfolio_dir}")
...
def main():
    ...
    parser.add_argument("input_path", help="Path to the experiment directory (e.g., experiments/brain_tumor/test_8/seed_42)")
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--seeds",
        help="Comma-separated list of seeds to merge",
        type=str
    )
    ...
    try:
        if args.merge_runs:
            if not args.experiment_names or not args.seeds:
                raise ValueError("Both --experiment-names and --seeds are required when using --merge-runs")
