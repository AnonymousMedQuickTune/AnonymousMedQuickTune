...
class NePSQuickTuneAdapter:
            ...
            with open(self.input_path, 'r') as file:
                ...
                for line in file:
                    ...
                    if line.startswith('Loss:'):
                        ...
                        try:
                            config_dict = eval(config_str)  # Safe since we know the format
    ...
    def create_dataframes(self, results: List[Dict[str, Any]]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        cost_data = {
            'cost': [r['number_of_epochs'] for r in results]
        }
        cost_df = pd.DataFrame(cost_data)
        
        return config_df, curves_df, cost_df

    def save_dataframes(self, config_df: pd.DataFrame, curves_df: pd.DataFrame, 
...
def main():
    ...
    parser.add_argument(
        "input_path",
        help="Path to the NePS output file"
    )
    parser.add_argument(
        "--output-dir",
