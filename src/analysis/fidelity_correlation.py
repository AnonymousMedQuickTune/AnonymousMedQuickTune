...
def parse_neps_output(file_path):
    ...
    for pair, configs in fidelity_pairs.items():
        print(
            f"Fidelity pair {pair} found in configs: {configs} (total: {len(configs)})"
        )
...
def analyze_correlations(config_losses):
    ...
    if len(fidelities) < 2:
        print(
            f"Found only {len(fidelities)} fidelities. At least 2 fidelities are required for correlation analysis."
        )
        return []
    ...
    for i in range(len(fidelities)):
        for j in range(i + 1, len(fidelities)):
            ...
            if len(pairs) < 2:
                print(
                    f"Warning: Skipping fidelity pair ({fid1}, {fid2}) - found only {len(pairs)} pairs, need at least 2"
                )
                continue
...
def plot_correlations(results, output_dir="correlation_results"):
    ...
    with open(os.path.join(output_dir, "correlation_results.txt"), "w") as f:
        ...
        for result in results:
            ...
            f.write(
                f"Pearson correlation: {result['pearson_correlation']:.3f} (p-value: {result['pearson_p_value']:.3f})\n"
            )
            f.write(
                f"Spearman correlation: {result['spearman_correlation']:.3f} (p-value: {result['spearman_p_value']:.3f})\n"
            )
...
if __name__ == "__main__":
    file_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "experiments/gist/test_budget-corr_0/seed_42/NePS_output/all_losses_and_configs.txt"
    )
